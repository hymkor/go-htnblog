...
- これまでドラフトのヘッダ部分と本文の markdown の間を`---` で区切っていたが、ヘッダ部分全体を`` ```header `` と `` ``` `` で囲むようにした
    - ヘッダの開始部分は `-header-open XXXXX` 終了部分は `-header-close YYYYY` で変更できるようにした。
    - `htnblog export` で出力されるバックアップテキストも、この指定に準拠する。  
      (というか、`htnblog export` で出力されるバックアップテキストを無加工で、そのまま、GitHub へ置けるよう、このフォーマットへ変更した)
...
- 10件よりも昔のエントリを編集などで指定できなかった不具合(v1.0.3)を修正
...
- htnblog.exe:
    - `htnblog list`: タイトル前に `[カテゴリ名]` をならべるようにした
